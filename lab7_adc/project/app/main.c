...
int main(void)
{
    /// STUDENTS: To be programmed
		...
		while(1){
			...
			uint8_t hexVal = CT_HEXSW & 0xF; 
			uint8_t resolution = hexVal % 4;	
			
			//adc_resolution_t resolution = ADC_RES_6BIT;
			uint16_t result = adc_get_value(resolution);
			
			CT_SEG7->BIN.HWORD = result;
			...
			CT_LED->HWORD.LED15_0 = displayVal;
			
			display_on_lcd((3300 / pow(2, 6 + 2*(3-resolution))) * result); // I stole that from wederma4
		}
